...
export default {
  ...
  data () {
    return {
      currentIndex: -2, // 默认存储的索引值, 用来存储鼠标进入某个菜单后的当前索引值  
      isShowHomeFirst: true // 默认显示一级分类列表
    }
  },
  computed: {
    // 通过辅助函数mapState获取home模块中三级分类信息数据
    ...mapState({
  ...
  // 页面加载之后
  mounted () {
    // 提交对应的action, 获取三级分类信息数据
    // 优化请求的次数(因为每次加载别的页面都会发送请求)所以需优化请求的次数
...
/*
  鼠标在一级菜单上切换, 出现了卡顿的问题--> 多层v-for嵌套的同时, 内部生成了大量的路由链接
